/* =============================================================
   HRA ACCOUNTANT – MAIN JAVASCRIPT (ULTRA-PREMIUM, ALL FEATURES)
   ============================================================= */
(function () {
  'use strict';

  /* ---------- CUSTOM SMOOTH SCROLL (smooth & slow, no stutter – Opera fixed) ---------- */
  const LERP_FACTOR = 0.1;   // premium glide speed
  let targetScrollY = window.scrollY;
  let animating = false;

  // Disable CSS smooth‑scrolling completely
  ...
  function animateScroll() {
    ...
    // Lerp towards target – explicit behavior: 'auto' prevents Opera’s internal smoothing
    window.scrollTo({ top: current + diff * LERP_FACTOR, behavior: 'auto' });
    requestAnimationFrame(animateScroll);
  }

  // Wheel handler – normalise delta for all browsers
  window.addEventListener('wheel', (e) => {
    e.preventDefault();
    let delta = e.deltaY;
    if (e.deltaMode === 1) delta *= 16;   // approximate line height for line-mode mice
    updateTarget(delta);
  }, { passive: false });
  ...
  // (Canvas removed from HTML, so this function does nothing, but kept for compatibility)
  function initParticles() {
    if (particlesInitialised) return;
    const canvas = document.getElementById('particle-canvas');
    if (!canvas) return;   // exits gracefully
    // ... rest of original particle code would be here, but omitted as canvas is removed
  ...
  /* ---------- PREMIUM 3D TILT & GLOW TRACKING (applied to all interactive cards) ---------- */
  function applyTiltToCards() {
    const cards = document.querySelectorAll('.feature-card, .package-card, .why-card, .value-card, .testi-card, .service-block, .rstep, .contact-item');
    cards.forEach(card => {
  ...
  function showPage(id, skipHash = false) {
    ...
    if (id === 'service') {
      ...
      requestAnimationFrame(() => {
        ...
        setTimeout(applyTiltToCards, 150);
      });
      closeMobile();
      updateTitle('service');
    ...
    requestAnimationFrame(() => {
      ...
      setTimeout(initReveal, 100);
      setTimeout(applyTiltToCards, 150);
      if (id === 'home') initParticles();   // harmless (canvas missing)
    });
    updateTitle(id);
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // Apply tilt and reveal on initial load
    applyTiltToCards();
